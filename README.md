...
A simple color generator using Values.js, HTML color input, and React Toastify.

[Try Website](https://simple-color-gen.netlify.app/)
...
In the Form component, I am using color state value and a form with an two input fields (color and text). Next, handleChange function to update the color state value and a handleSubmit function to add the color to the list.

[HTML Color Input](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/input/color)

#### Values.js

Install the values.js library and use it to generate a list of colors in the App component. I already included in the package.json.
...
In the App component, I create logic to generate a new list of colors when the user submits a new color value. Using react-toastify to display an error message if the values.js library cannot generate a color, and also display a toast message if the user tries to submit an empty value.

#### Clipboard

In the SingleColor component, I used click handler that saves the hex value of the color to the browser clipboard.
